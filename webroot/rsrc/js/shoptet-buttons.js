...
(function() {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (commands !== false) {
        ...
        if (option && commands[command].visible) {
          ...
          button.addEventListener('click', (e) => {
            ...
            if (!button.classList.contains('shoptet_button_active')) {
              ...
              if (commands[command].value === 'column' && typeof workboardColumnsSetUp === 'undefined') {
                setTimeout(() => {
                  setupWorkboardColumns();
      ...
      const setupWorkboardColumns = () => {
        let optgroup = form.querySelector('optgroup[label="Workboards"');
        if (optgroup) {
          const buttons = document.createElement('div');
          buttons.classList.add('shoptet_buttons');
          ...
          options.forEach(option => {
            ...
            button.addEventListener('click', e => {
              ...
              option.setAttribute('selected', true);
              let ev = new CustomEvent('change');
              optgroup.parentNode.dispatchEvent(ev);
            });
